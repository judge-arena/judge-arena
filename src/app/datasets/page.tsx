...
export default function DatasetsPage() {
  ...
  const handleCreateLocal = async () => {
    ...
    try {
      ...
      let samples: Array<{ input: string; expected?: string }> | undefined;
      if (localMode === 'inline') {
        samples = localSamples.filter((s) => s.input.trim());
      } else if (localMode === 'paste' && localData.trim()) {
        try {
          const parsed = JSON.parse(localData);
          if (Array.isArray(parsed)) {
            samples = parsed.map((item: any) => ({
              input: item.input || item.prompt || item.question || JSON.stringify(item),
              expected: item.expected || item.answer || item.output,
            }));
          }
        } catch {
